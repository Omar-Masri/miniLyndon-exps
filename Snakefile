...
PLAIN_READ_DATASETS = ["Ecoli10", "Ecoli25", "Ecoli50", "Ecoli100"] + expand(
    "Celegans_UNSB01.{type}.{cov}",
    type=["perfect", "ccs"],
    cov=[10, 25, 50],
)
...
ALL_REPORTS = ["multiqc/all.html"] + expand(
    "multiqc/{dataset}/report.html",
    dataset=(
        ["Ecoli_K12_DH10B", "Celegans_UNSB01"]
        + expand(
            "Ecoli_K12_DH10B/{reads}",
            reads=["Ecoli10", "Ecoli25", "Ecoli50", "Ecoli100"],
        )
        + expand("Celegans_UNSB01/Celegans_UNSB01.{type}", type=["perfect", "ccs"])
        + expand(
            "Celegans_UNSB01/Celegans_UNSB01.{type}.{cov}",
            type=["perfect", "ccs"],
            cov=[10, 25, 50],
...
rule read_stats_all:
    input:
        read_stats=expand("Reads/{dataset}-stats.csv", dataset=PLAIN_READ_DATASETS),
    output:
        all_stats="Reads/all-stats.csv",
    conda:
        "envs/csvkit.yml"
    shell:
        """csvstack --filenames {input.read_stats} > {output.all_stats}"""
...
rule read_stats:
    ...
    output:
        stats="Reads/{reads}-stats.csv",
    shell:
        """
        python3 scripts/fastaq_stats.py < {input.reads} > {output.stats}
        """
...
rule multiqc:
    input:
        targets=TARGETS,
    output:
        html="multiqc/all.html",
    params:
        prefix=lambda wildcards, input, output: subpath(output.html, parent=True),
    conda:
        "envs/multiqc.yml"
    shell:
        """multiqc -o "{params.prefix}" -d  -s -i 'All datasets' -n all --force {input.targets:q}"""


rule multiqc_dataset:
    input:
        targets=lambda wildcards: [
            str(p)
            for p in Path("Results").glob("**/quast")
            if str(p).startswith("Results/" + wildcards.dataset)
        ],
    output:
        html="multiqc/{dataset}/report.html",
    params:
        prefix=lambda wildcards, input, output: subpath(output.html, parent=True),
    conda:
        "envs/multiqc.yml"
    shell:
        """multiqc -o "{params.prefix}" -d  -s -i '{wildcards.dataset}' -n report --force {input.targets:q}"""
...
rule unzstd:
    input:
        zst="Reads/{read}.fa.zst",
    output:
        fa="Reads/{read}.fa",
    shell:
        """
        unzstd --long=31 {input.zst}
        """
